...
The goal of the project Inception is to set up an architecture in Virtual Machine(VM), so the user is is able to host and manage a blog website by using Docker containers, networks and volumes. Project also requires documents as README.md, DEV_DOC.md and USER.md, that reflect how the project was done and how to use manage the docker containers.
...
Reboot VM and log in to the user account.
	- configure domain name so it points to the local IP address: echo "127.0.0.0 vuljas.42.fr" >> /etc/hosts
...
Execution:
	...
	- Start containers without builiging new images: make up
...
A Project description [ section must also explain the use of Docker and the sources included in the project. It must indicate the main design choices, as well as a comparison between:
	...
	**Secrets vs Environment Variables**
	...
	◦ Env vars are not encrypted							|	◦ Avoid exposing sensitive data
