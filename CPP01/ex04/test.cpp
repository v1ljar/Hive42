...
int main()
{
	...
	result = run_test(base, "./tests/basic_test", "This is the world\nworld world\nSo long, world", "world", "universe");
	if (result ? std::cout << "\t✅ Basic test passed!\n\n" : std::cout << "\t❌ Basic test failed!\n\n")
	
	result = run_test(base, "./tests/no_space_test", "This is the world\nworld world\nSo long, world", "' '", "''");
	if (result ? std::cout << "\t✅ No space test passed!\n\n" : std::cout << "\t❌ No space failed!\n\n")
	
	result = run_test(base, "./tests/alphabet", "1:2:3lmnopqrstuvwxyz", "1:2:3", "abcdefghijk");
	if (result ? std::cout << "\t✅ Alphabet test passed!\n\n" : std::cout << "\t❌ Alphabet test failed!\n\n")
	
	result = run_test(base, "./tests/empty", "1:2:3lmnopqrstuvwxyz", "1:2:3lmnopqrstuvwxyz", "''");
	if (result ? std::cout << "\t✅ Empty test passed!\n\n" : std::cout << "\t❌ Empty test failed!\n\n")
